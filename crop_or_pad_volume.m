...
function adjusted_volume = crop_or_pad_volume(volume, target_size, pad_value)
...
%       pad_value   - Scalar, the value to use for padding (default is 0)
%
%   Output:
...
% Set default value for pad_value if not provided
if nargin < 3
    pad_value = 0;
end
...
elseif x < target_size(1)
    ...
    adjusted_volume = padarray(adjusted_volume, [pad_before 0 0], pad_value, 'pre');
    adjusted_volume = padarray(adjusted_volume, [pad_after 0 0], pad_value, 'post');
end
...
elseif y < target_size(2)
    ...
    adjusted_volume = padarray(adjusted_volume, [0 pad_before 0], pad_value, 'pre');
    adjusted_volume = padarray(adjusted_volume, [0 pad_after 0], pad_value, 'post');
end
...
elseif z < target_size(3)
    ...
    adjusted_volume = padarray(adjusted_volume, [0 0 pad_before], pad_value, 'pre');
    adjusted_volume = padarray(adjusted_volume, [0 0 pad_after], pad_value, 'post');
end
end
